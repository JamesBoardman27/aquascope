...
- [x] The keyless crew, the recorded studies and the plan-quality benchmark (September 2026): bring-your-own-model entry points so a page can run the crew's prompts on the device model, a keyless Consultant that asks real questions and keyless follow-ups that add steps, twelve recorded Sonnet 5 studies replayed keyless on the Space, HydroGym Phase 2 with twenty-five expert reference plans scoring the Methodologist against the tree, and a sturdier Explorer (Stop that restarts the worker, studies that persist and resume, the Excel path) ([#363](https://github.com/Rekin226/aquascope/issues/363), #364 to #367). Still open there: the device-class row of the benchmark (needs a free small-model endpoint), a model that honours the tree's data-driven declines, the launch itself
