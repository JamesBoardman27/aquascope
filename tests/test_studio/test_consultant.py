...
def test_rules_write_the_brief_and_ask_nothing_when_every_field_has_a_default():
    ...
    assert any("design flow" in a for a in b.assumptions), "the defaults taken are declared as assumptions"
...
def test_a_supply_problem_asks_for_the_demand_and_the_answer_is_coerced():
    ...
    assert ids == ["demand_m3s", "demand_ml_day"] and "1." in msg.text and "just go" in msg.text
    msg2 = consultant.consult(ws, None, "2.5 cubic metres per second")
    assert ws.brief.ready and msg2.kind == "brief"
    assert ws.brief.intake["demand_m3s"] == 2.5 and ws.brief.intake["use"] == "municipal"
    assert ws.brief.questions[0].answer == 2.5
...
def test_no_playbook_asks_which_and_the_answer_picks_one():
    ...
    consultant.consult(ws, None, "drought_status please")
    assert ws.brief.playbook == "drought_status" and ws.brief.kind == "drought" and ws.brief.ready
