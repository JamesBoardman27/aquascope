...
- USGS keyed path: `stateCd` / `countyCd` are translated to the ANSI codes the OGC API expects (`MD` to `24`, `24033` to `033` with the implied state `24` if `state_code` is not in the parameter list). Comma-separated `stateCd` / `countyCd` / `huc` values trigger a warning since the OGC API takes single values for these fields - we choose to pass the first value of the comma-seperated list as the parameter in these instances rather than returning nothing. (#160)
