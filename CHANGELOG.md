...
### Added
- **Catchments everywhere: BasinATLAS in the Archive** (`aquascope.archive.basins`, `aquascope basins`, `basins.yml`, MCP `describe_catchment`, Explorer): the level-12 sub-basins of HydroATLAS v1.0 / BasinATLAS (Linke et al. 2019, CC BY 4.0) are published under `basins/` as an indexed FlatGeobuf (point lookups over HTTPS), a topology parquet (routing + representative points), the full attribute parquet and PMTiles. `describe_catchment(lat, lon)` gives the sub-basin, the upstream count/area and the catchment's attributes (BasinATLAS's own upstream-aggregated fields, else area-weighted over the upstream set); `aquascope basins at|upstream|build`; the analyst gets the tool; the Explorer finds the sub-basin in the browser, walks upstream in DuckDB-WASM, highlights the upstream sub-basins and shows the attribute card for any station or point ("Basins" toggle in the legend). New `basins` extra (pyogrio, geopandas, shapely). (#189, #100)
- **Explorer: catchments** (`explorer/app.js`): for every USGS gauge and any point clicked in the US, the upstream drainage basin is drawn on the map with its area, from the USGS Network Linked Data Index over NHDPlus V2 (public domain, fetched from api.water.usgs.gov, nothing stored); the method and source join the citations panel. HydroBASINS was ruled out after reading its licence (no stand-alone redistribution, end-user licence required). (#189)
- **Explorer: Ask ✨, the Analyst in the page** (`explorer/`): a panel where a plain-language question is answered by `aquascope.ai_engine.analyst.ask` running inside the browser worker: the model (Groq, Hugging Face, OpenAI, Mistral, OpenRouter or any OpenAI-compatible endpoint, bring your own key) picks the tools, aquascope runs them on the catalog already loaded in the tab plus the agencies' APIs, and the rendered report ends with the Data and Methods sections; stations touched become chips that open on the map; Markdown copy/download. The key goes from the tab straight to the provider (the page has no server). New `aquascope.ai_engine.llm_transport.UrllibChatClient`: a dependency-free OpenAI-compatible chat client (urllib, works under pyodide-http) that `ask` and `ingest --llm` fall back to when the `openai` package is missing, so `aquascope ask` no longer needs the `llm` extra; providers `mistral` and `openrouter` added; `aquascope.archive.catalog.set_catalog()` lets a host hand the catalog to `find_stations` without pyarrow/httpx. (#189)
- **Archive Phase 2: more variables and Parquet bundles** (`aquascope harvest bundles`, `aquascope.archive.bundles`): the observation harvest now mirrors USGS gage height (feet converted to metres), and UK EA water level (daily max where no daily mean is published), daily rainfall totals and borehole groundwater levels (mAOD, manual dips or logger), each variable with its own budget and manifest cursor (`obs/manifest.json` version 2, keyed `source/variable`, migrated in place). After each run the per-station folders are rolled into one Parquet per variable and source (`obs/<variable>/<source>.parquet`: `station_id, date, value`, joinable to `stations.parquet`), readable with `pd.read_parquet`, DuckDB or `aquascope.archive.load_observations(source, variable)`. `fetch_series`, `explore.analyze_station` and the MCP `analyze_station` / `get_timeseries` tools take a `variable` argument; UK EA measure selection is variable-aware and labels boreholes `groundwater_level`; the trend test now judges year coverage against the record's own sampling so a 40-year monthly borehole record gets a Mann-Kendall result. (#188)
- **BOM (Australia) collector** (`collectors/bom.py`): streamflow, water level, storage, and groundwater level from the Bureau of Meteorology's Water Data Online portal (KISTERS WISKI/KiWIS API), covering ~8,000 stations. No API key required. Thanks @adjenk (#181, closes #4)
- **Shared source registry, extended with coverage and terms** (`aquascope/registry.py`): every collector is now described once, with `agency`, `country`, `variables` (a controlled vocabulary in `aquascope.schemas.station.VARIABLES`), `supports_bbox`, `supports_station_lookup`, `output_model`, `license`, `redistributable` and `attribution`. The CLI (`collect`, `list-sources`, `stations`), the dashboard Collect page, `aquascope.collect()` and the two Taiwan WRA groundwater collectors (previously reachable from Python only) all read from it. `redistributable` defaults to `False` and needs a real licence id; a new `tests/test_registry.py` guards that every collector class is registered, every key builds, and the station-catalog flag matches the code. Registry extraction by @laishettikarthik-tech (#163); extension per #187.
- **Archive Phase 1: daily observations** (`aquascope harvest obs`, `aquascope.archive.observations`): budgeted, incremental per-station harvest (`obs/<variable>/<source>/<station_id>.csv.gz`, `obs/manifest.json`) for USGS, UK EA, Hub'Eau discharge and Taiwan CWA rainfall, all `redistributable=True`; a station is refreshed after 30 days; failures never sink a run. The weekly workflow syncs the previous tree, harvests a budget per source and publishes once. `aquascope.explore.fetch_series` reads a station's archive file first and falls back to the agency, so the Explorer and the MCP tools stop re-hitting agencies for mirrored stations. (#188)
- **Explorer: click anywhere** (`aquascope.explore.anywhere`, `#p=<lat>,<lon>` permalinks): ERA5 rainfall/temperature, FAO-56 ET0 and aridity class, monthly climate chart, GloFAS modelled discharge with an indicative return-period table, and the nearest gauges, from Open-Meteo, keyless. (#189)
- **The Analyst** (`aquascope ask`, `aquascope.ai_engine.analyst`): a tool loop over the MCP tool functions for any OpenAI-compatible endpoint with tool calling (OpenAI, Groq, Hugging Face router, Ollama); the Markdown report ends with **Data** and **Methods and citations** sections assembled from tool results, never from the model. (#113 follow-up, direction review "ask, get the work done, see the work")
- **Ingest anything** (`aquascope ingest`, `aquascope.ingest`): CSV/TSV/Excel/JSON export → heuristic (or LLM-proposed, heuristic-validated) column mapping → deterministic apply (sentinels dropped before unit conversion, duplicates, timezone) → QA report (coverage, gaps, spikes, negatives, per-year coverage) → clean `date,value` CSV in SI units plus `.qa.json` / `.qa.md`, and `analyze_series` on the result.
- **Harvest self-reporting** (`.github/scripts/harvest_issues.py`): the scheduled harvest opens one `collector-health` issue per failing source with the error and a deterministic diagnosis (404 / 429 / TLS / timeout / 5xx / format), comments while it keeps failing, and closes it when the source recovers.
- **MCP server** (`aquascope mcp`, `aquascope.mcp_server`, new `mcp` extra): seven tools for MCP clients (Claude Desktop / Claude Code / Cursor): `list_sources`, `find_stations` (searches the published catalog, no agency call, nearest-first from a lat/lon or a bbox), `get_timeseries` (collector fetch, resampled and thinned), `analyze_station` and `flood_frequency` (`aquascope.explore` with citations, optional bootstrap CI), `describe_methods`, `archive_health`; two resources. Response sizes are bounded by design. Works with the Python SDK 1.x (`FastMCP`) and 2.x (`MCPServer`). `aquascope.archive.catalog.load_stations()` reads the Archive parquet into a daily local cache for any consumer. Verified end to end with the SDK's stdio client. (#113)
- **`aquascope.explore`**: the `(source, station_id) -> answer` entry point (`analyze_station`, `fetch_series`, `analyze_series`, `flood_ci`, `to_csv`) lives in the package now, shared by the Explorer worker, the CLI and the MCP server.
- **Hub'Eau elaborated series** (`HubeauHydrometrieCollector.fetch_raw(elaborated="QmnJ")`): the `/obs_elab` endpoint (daily/monthly means and extremes, multi-decade history) normalises into the same `StreamflowReading` / `WaterLevelReading` models as the real-time feed. The Explorer uses it first, so French stations get flood frequency too.
- **USGS station names without a key**: when the OGC `monitoring-locations` walk is throttled, `stations()` falls back to the keyless NWIS site service (per state, or per bbox), so the harvested catalog keeps names.
- **AquaScope Explorer, Phase 0** (`explorer/`, live at https://rekin226-aquascope-explorer.static.hf.space/): a static page with no server. MapLibre map of every station in the Archive (DuckDB-WASM reads the GeoParquet in place, GeoJSON fallback), search, permalinks. Clicking a station fetches the observed record through aquascope's own collectors running in a Pyodide Web Worker and shows the hydrograph, annual maxima, GEV (L-moments) and Log-Pearson III return levels with 90 % confidence limits, an on-demand bootstrap GEV band, the flow-duration curve, a Mann-Kendall trend, a CSV download and a methods-and-citations panel. Full daily records for USGS and UK EA; real-time windows for Hub'Eau, PEGELONLINE and Ireland OPW; daily rainfall for Taiwan CWA. `explorer/analysis.py` runs unchanged in CPython and is tested in `tests/test_explorer/`. Deployed by `.github/workflows/explorer.yml` (HF static Space) and under `/explorer/` on the docs site. (#189 Phase 0)
- USGS collector: the per-record catchment-area lookup is memoized per station, so a throttled OGC endpoint no longer costs a retry cycle on every row of a long daily record.
- **The Archive, Phase 0** (`aquascope.archive`, `aquascope harvest stations`, `.github/workflows/harvest.yml`): every station catalog is harvested into GeoParquet 1.0 (`stations.parquet`, WKB points, WGS84), a slim `stations.geojson`, a per-source `health.json` and a regenerated dataset card, and published weekly to the public Hugging Face dataset [`Rekin226/aquascope-gauges`](https://huggingface.co/datasets/Rekin226/aquascope-gauges). First run: 45,919 stations from six sources; DuckDB reads the parquet in place over HTTPS. Observations are not mirrored yet, and when they are, only for sources whose registry entry says `redistributable=True`. New `archive` extra (`pyarrow`, `huggingface_hub`), docs page `docs/archive.md`. (#188 Phase 0)
- **Station catalogs** (`BaseCollector.stations()`, `aquascope.find_stations()`, `aquascope stations`): six sources answer "which stations do you have, where, measuring what": USGS (OGC `time-series-metadata` + `monitoring-locations`, keyless), UK EA (`id/stations.json`), Hub'Eau (`referentiel/stations`), PEGELONLINE (`stations.json`), Ireland OPW (waterlevel.ie GeoJSON) and Taiwan CWA (CODIS `station_list`, 1,265 climate stations). Results are `Station` records (id, name, lat/lon, variables, period of record, agency URL) with `bbox` / `variable` / `max_items` filters; `station_catalogs()` keeps per-source failures visible so "no stations" and "endpoint failed" stay distinguishable. `aquascope stations --bbox w,s,e,n --variable discharge --format geojson|csv|json` writes the result. (#187; first slice of the archive and explorer epics #188 and #189)
...
### Fixed
- **USGS OGC pagination could spin forever** (`collectors/usgs.py`): `next` links were re-fetched with `params={}`, which makes httpx rebuild the query string and drop the cursor, so page one came back from the disk cache in a silent infinite loop (the second CI harvest run hung on the sixth station). Next links are now fetched as-is and a repeated cursor stops the loop; the obs step also has its own timeout.
- **USGS keyed path filters** (`collectors/usgs.py`): with an API key, `station_id` / `parameter` / `stateCd` / `countyCd` / `huc` are now mapped onto the OGC daily collection's `monitoring_location_id` / `parameter_code` / `state_code` / `county_code` / `hydrologic_unit_code` instead of being ignored (a per-station fetch used to crawl the whole national collection). Found by the first CI harvest run. (closes #160)
