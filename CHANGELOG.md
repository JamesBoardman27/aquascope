...
- **Greece: live gauges from OpenHi.net** (#379). `GreeceOpenhiCollector` / source key `greece_openhi` reads the Enhydris 3 REST API of the Open Hydrosystem Information Network run by ITIA at NTUA, the live counterpart to the archival `greece_hydroscope`. 65 stations, 37 of them reporting this year, across discharge, stage, rainfall, air and soil measurements and an in-water chemistry set; discharge at the three Mandra stations is 15-minute and current to today. `stations()` filters by bbox and variable and resolves each station's variables from its groups; `fetch_raw()` pushes `start_date` / `end_date` to the API (one Mandra series is 12.9 MB whole and 27 KB for a five-day window) and `latest_only=True` uses `/bottom/` so a liveness sweep costs one row per series. Discharge normalises to `StreamflowReading` in m3/s, stage to `WaterLevelReading` in metres (converted from cm where the API says cm), rainfall and the atmospheric and soil variables to `ClimateReading` with the station altitude carried through, and the chemistry to `WaterQualitySample`. Four upstream shapes are counted and logged separately rather than collapsed into one failure (#165): a populated series, one catalogued with a zero-byte body, one returning timestamps with a blank value column, and one refused with 401. Every list endpoint is paginated and the `next` walk is followed, EWKT geometry is parsed lon-before-lat with non-4326 geometries rejected rather than read as degrees, and an empty raw series falls back to the group's aggregated roll-up. Licensed CC BY-SA 4.0, so `redistributable=True` and the archive may mirror the observations. 38 tests, every fixture captured from a real response.
- **Greece: the Hydroscope national databank** (requested on Reddit). `GreeceHydroscopeCollector` / source key `greece_hydroscope` reads the open Enhydris REST API behind [hydroscope.gr](https://hydroscope.gr), the National Databank for Hydrological and Meteorological Information, across the three federated instances that actually serve observations: the Ministry of Environment and Energy, the Hellenic National Meteorological Service and the Ministry of Rural Development and Food. 704 stations with data (601 rainfall, 86 river stage, 20 discharge), spanning 1904-2019, keyed by the same composite ids the national catalog uses so a station id round-trips to its page on main.hydroscope.gr. `stations()` filters by bbox and variable, and `fetch_raw()` pushes a two-sided date window to the server as a path suffix rather than pulling a 60-year daily file to slice it locally. Discharge normalises to `StreamflowReading` in m3/s, stage to `WaterLevelReading` in metres, rainfall to `ClimateReading`; the conversion reads the unit the downloaded file declares, because the catalog's `unit_of_measurement` disagrees with it (series advertised as m3/s download as l/s), and a series whose unit is not convertible is dropped rather than assumed. Series the catalog lists but never populated (65 of the 67 discharge series on one instance return `Count=0`) are skipped from their null start date without a request, and Greek Grid geometries are rejected rather than misread as degrees. The Public Power Corporation instance (639 stations in the national catalog) is not emitted: its host does not resolve and the catalog returns empty bodies for its series. `redistributable` stays `False` pending published terms, so the archive mirrors the catalog with a link back, not the observations.
...
- **Studio: bring your own model, and the keyless crew** (#363, #364). `Studio.say(text, proposed=...)`, `Studio.approve(plan=...)` and `Studio.narrate(sections=...)` take a brief, a plan or section prose written by any model the caller ran and treat them exactly like the crew's own: the validator with its repairs on the plan (a guessed gate path corrected, a wrong method dropped, a choice a tool rejects replaced, a station the site's inventory does not know refused), the tree when nothing valid remains, the deterministic checks on the prose (a sentence with a number not in the results is dropped and counted), the footer saying who wrote what. `consultant_context`, `methodologist_context` and `author_context` return the exact context and system prompt each role would send, and `python -m aquascope.studio.prompts` exports them as `explorer/prompts.json`; the same over MCP (`studio_narrate`, `studio_context`, `proposed` and `plan` on `studio_say` / `studio_approve`). Keyless, the Consultant asks real questions from the gaps in the brief (the decision with the playbook's options, the return period, the drought period, which column of an upload; at most three; "just go" lists the defaults as assumptions), and a follow-up can add steps from a rule table (another or the upstream gauge, the donors, the flow duration, the trend, SPI and SPEI, the baseflow, the GloFAS cross-check), validated the same way. Key numbers carry both fits at every return period asked, every drought timescale with its class, the signature bands and the reliability rows.
- **What the plan-quality benchmark taught the Methodologist** (#367). A playbook's own decline rule (an inundation map, a cause without pumping data, a reservoir yield, a day-by-day schedule, a health verdict beyond the samples) is honoured before any model is asked, and a model may decline instead of planning; the table tools are listed to the Methodologist without an upload, so the water-quality index is reachable through a samples step; the supply screening's ungauged mode is reachable; a plan may only name stations the site's inventory knows.
- **HydroGym Phase 2, plan quality** (#367). Twenty-five reference plans written by hand across the seven problem kinds and three off-tree briefs (`aquascope/gym/plans/`, each with its reconnaissance saved so scoring needs no network; three of them declines by rule), a scorer with a documented formula (tool, method and gate coverage, forbidden tools and methods, extraneous steps, wrong and missed declines, validator errors on the first try, tokens and cost), `aquascope gym plans list | show | validate | score | rescore` and `aquascope gym bench --agent tree | methodologist | file`, the leaderboard with the plan-quality columns, `docs/hydrogym.md` extended, results and a deposit package under `aquascope/gym/results/2026-09-07/`. The first run scored the tree at 0.97 and the Methodologist at 0.64 on Claude Sonnet 5 and 0.66 on Claude Haiku 4.5, because with a model the Studio never declined an ill-posed brief; after the engine fixes below the same cases score 0.96 and 0.93, with every ill-posed brief declined (`aquascope/gym/results/2026-09-08/`).
- **Explorer: the device model as the crew, and a sturdier Study mode** (#365). When Ask's on-device model is already loaded and no key is used, Study runs the exported prompts on it: the plan (checked in the worker, the tree when it fails, the card saying who planned it) and the prose after the run (checked by the Critic; the board says how many sentences the checks dropped); never a download it did not ask for. Stop terminates the worker and re-initialises it with the plan kept; a study persists in the browser (IndexedDB, the last five) and offers "Resume the last study" after a reload; a dropped `workspace.json` resumes a study; an .xlsx drops in as a table; the Study modules load on first use (the first visit is 33 KB of JavaScript lighter). Worker ops `narrate`, `context`, `check_plan`, `prompts`, and `proposed` / `plan` on the existing ones.
- **Recorded studies on the Space** (#366). Twelve studies run on Claude Sonnet 5 with the crew (Kingston flood, Potomac supply, Taichung drought, a Cambridge Chalk borehole, the Niger at Bamako ungauged, Nairobi and the Sintra hills as bare points, the Murrumbidgee at Wagga Wagga on the regional path, Little Falls water quality, a user's table, the Garonne irrigation, the Loire drought) recorded under `explorer/showcase/studies/` with their figures, reports and study.yaml (`aquascope studio-showcase record | list`), offered on the Study board as chips: the recorded prose labelled with the model, the date and the cost, and "Re-run live" running the recorded plan keyless in the browser through the validator and the gates. Honest outcomes are kept as recorded (a failed cross-check under "not established", a regional path where the agency serves no record).
...
- **CAMELS benchmark, Phase 2: the harness** (#56). `benchmarks/camels_benchmark.py` runs AquaScope's own methods over the 10 benchmark catchments with no network access — flood frequency (`gev`, `gev_lmoments`, `lp3`), baseflow (`lyne_hollick`, `eckhardt`) and the full signature set — and compares every number against the published CAMELS attributes and the Phase 1 reference quantiles. One `results.json` is the single source of truth; `results.md` and `results.html` are renderings built by `ReportBuilder` (with the #334 software citation) that read the JSON. `python -m benchmarks.camels_benchmark --output-dir benchmark-output` runs it, `--from-json results.json` re-renders a recorded run into `results.md` / `results.html` (plus a JSON copy) without recomputing a single number (`--gauge-id`, `--no-reports`, `--strict`). Tolerances are decided up front and written into `metadata.tolerances` (signatures ±25 % relative, BFI ±0.15 absolute, peak month ±2 months, FFA ±20 % relative, plus aggregate q_mean-NRMSE / BFI-PBIAS / FFA gates of 25 % / 25 % / 20 %). GEV is compared estimator-to-estimator against the reference MLE: where the reference fit is unstable (`mle_stable: false`, 5 gauges) the mismatches are classified `data_limitation` and surfaced in the summary as cross-implementation findings rather than hidden. Per-stage execution time is recorded. Committed examples live in `examples/camels_benchmark/`, local runs write to `benchmark-output/`, each catchment also carries a signature-integrity block (q5 < median < q95, runoff ratio in (0, 1), positive recession constant, negative FDC slope, all fields populated — the invariants the old `examples/validation` suite checked) recorded in the JSON and surfaced in the report, the results shape is defined once as Pydantic models (`benchmarks/results_models.py`) from which `build_results()` validates every run and from which the committed JSON Schema `benchmarks/results.schema.json` (draft 2020-12) is generated: `build_results()` raises on a type leak or missing field and `--from-json` refuses to render a file the models reject, so consumers can hold the published shape via `cb.validate_results()`, and 24 tests cover the schema, the gates, the summaries, the JSON and the CLI.
- **CAMELS benchmark, Phase 1: USGS peak-flow reference data** (#56). `benchmarks/fetch_peak_flows.py` fetches the annual peak records for the ten CAMELS catchments from the USGS NWIS `peaks` service via `dataretrieval`, screens them, caches them under `data/camels_benchmark/peaks/` and fits three independent extreme-value distributions — GEV-MLE and a basic log-Pearson III on log10 maxima (scipy) and GEV-L-moments (lmoments3, Hosking 1997) — to publish reference quantiles at [2, 5, 10, 25, 50, 100]-year return periods in `ffa_reference.json`. The series is screened before any fit: rows carrying USGS qualifier codes 3/4/8 (dam failure, censored) and non-positive, sentinel or undated values are dropped, and the series is collapsed to one value per USGS water year, so the reference never feeds `log10` a non-positive value or counts a partial-year record as a flood year; every removal is tallied in the catchment's `data_quality` block. An unstable GEV-MLE (five of the ten gauges have an extreme shape under MLE) is kept but flagged `mle_stable: false` with a `RuntimeWarning`, since the independent L-moments fit is the usable cross-check there, and the LP3 label states plainly that it is a basic fit rather than Bulletin 17C. Outputs publish atomically: nothing is written until every gauge has been fetched, cleaned and fitted, and each file replaces its predecessor only once the whole run has succeeded, so a failed rerun never leaves a partial cache. `dataretrieval` and `lmoments3` are generation-only (`pip install "aquascope[benchmarks]"`), not runtime dependencies. 25 unit tests stub the network so the suite runs anywhere.
...
- **Greece is clickable in the Explorer, and the stage record is no longer a blend** (follow-up to #379). Both Greek sources list stations, so they were already pins on the Explorer map, but a click raised `no Explorer fetch path yet`; `fetch_series` now serves them, pushing the requested window down to the API (one OpenHi series is 12.9 MB whole) and trying discharge, stage then rainfall. Fixing that surfaced a real defect in `greece_hydroscope`: a station can hold several series for one quantity, and all of them were concatenated. Station 200082 has two ΣΤΑΘΜΗ series (1950-1983 and 1950-1982) plus a ΣΤΑΘΜΗ (ΠΛΗΜΜΥΡΑ) one, which came back as 515,650 rows carrying 11,219 duplicate timestamps with flood stage interleaved into the stage record; it is now 11,342 rows from the single best series. Exactly one series wins per station and variable: the most-preferred variable id (ordinary stage over flood stage, continuous ΠΑΡΟΧΗ over individual ΥΔΡΟΜΕΤΡΗΣΗ gaugings), then the longest record, with the rest logged rather than dropped silently. A new drift guard asserts every catalog source reaches a fetch path and names the two that still do not (`bom`, `brazil_ana`), so the next collector cannot ship pins that error on click.
- **The docs site builds again.** `mkdocs build --strict` had been failing since 2026-09-08, so nothing had deployed to GitHub Pages since. Two causes: `docs/hydrogym.md` linked the HydroGym discussion by a path outside the docs tree, which MkDocs cannot resolve (it is now the GitHub blob URL the other pages use for repo files), and `BrazilANACollector`'s docstring ran a prose paragraph straight on from its numpydoc parameter list, so griffe read "Credentials", "mode" and "series" as undeclared parameters (it is a `Notes` section now, and `legacy_client` is documented). The docs workflow now also builds on pull requests (deploying still only on main), so the next broken build fails the PR instead of the site going stale for days.
- **Stale counts across the README and its mirrors.** Every file that states a count had drifted from the code and from the others: the source count read 31 in the README, 29 in `docs/index.md`, 20 in `docs/features.md`, 21 in the French README and 20 in the `CITATION.cff` abstract, against 34 in the registry. Crops read 23 against the 26 in `KC_TABLE`, `docs/features.md` claimed 21 hydrological signatures against 22 and a 14-command CLI against 29, and the French README still advertised a 19-command CLI. The test badge and its prose copies read 1,000+ (820+ in features) where CI reports 2,525 passing on every supported Python version, so they now read 2,500+. Every count is derived from the code, and the drift guard checks each mirror by name instead of the README alone. The stale source lists (the "Pull water data" bullet, the regional bullets, the `docs/features.md` collection list) gained the collectors added since they were last written: NOAA NWPS, Colorado DWR/CDSS, ANA Hidroweb, Ireland OPW, Greece Hydroscope, PEGELONLINE, England's Environment Agency, Australia BOM, CAMELS-BR and Taiwan CWA.
...
- **FAO-56 crop coefficients (Kc, Kcb) and growth-stage lengths for millet, cassava and chickpea** (#21, #340, jddrtn); chickpea's stage lengths use the Lentil row of Table 11 as a proxy, since Table 11 has no chickpea row.
