...
- **USGS keyed OGC path now honours `station_id`, `parameter`, `stateCd`, `countyCd` and `huc` filters** (`collectors/usgs.py`): with an API key, `station_id` / `parameter` / `stateCd` / `countyCd` / `huc` are now mapped onto the OGC daily collection's `monitoring_location_id` / `parameter_code` / `state_code` / `county_code` / `hydrologic_unit_code` instead of being ignored (a per-station fetch used to crawl the whole national collection). Found by the first CI harvest run. Translating 2-letter state abbreviations to two-digit ANSI codes, five-digit county codes to three-digit ANSI codes, and adding the `USGS-` agency prefix where needed. Bare and `USGS-`-prefixed site numbers are treated identically on both paths. A state/county code that cannot be mapped to the ANSI standard now logs a warning and drops that filter instead of silently matching nothing, and a bare three-digit county code without a `stateCd` warns that the query will match that county in every state (a full five-digit FIPS code recovers the state from its prefix). A comma-separated list of state codes, county codes or HUCs (which the legacy NWIS API accepts but the OGC API rejects with an empty response) warns and keeps only the first value, since the OGC path takes one state/county/HUC per query; comma lists of monitoring locations and parameter codes are forwarded unchanged. (closes #160)
